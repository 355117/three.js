 ...
 *
 * 该材质使用基于非物理的 [Blinn-Phong]{@link https://en.wikipedia.org/wiki/Blinn-Phong_shading_model}
 * 模型来计算反射率。与 {@link MeshLambertMaterial} 中使用的Lambertian模型不同，
 * 这可以模拟具有镜面高光的光泽表面（如清漆木材）。`MeshPhongMaterial` 使用逐片段着色。
 ...
 *
 * 使用此材质比使用 {@link MeshStandardMaterial} 或 {@link MeshPhysicalMaterial}
 * 性能通常更好，但会牺牲一些图形精度。
...
class MeshPhongMaterial extends Material {
  ...
  constructor(parameters) {
    ...
    /**
     * 材质的镜面反射颜色。默认颜色设置为 `0x111111`（非常深的灰色）。
     * 这定义了材质的光泽程度和光泽的颜色。
     ...
     * 凹凸贴图实际上不会影响对象的几何形状，只影响光照。如果定义了法线贴图，这将被忽略。
     ...
     * 法线贴图不会改变表面的实际形状，只影响光照。如果材质的法线贴图使用左手坐标系约定创建，
     * 应该将 `normalScale` 的 `y` 分量取反以补偿不同的手性。
    ...
    /**
     * 位移贴图值在网格顶点上的偏移量。偏移量被添加到位移贴图的缩放采样中。
     * 如果没有设置位移贴图，此值不会被应用。
    ...
    /**
     * 镜面反射贴图值影响镜面表面高光的贡献程度以及环境贴图对表面的影响程度。
    ...
    /**
     * alpha贴图是控制表面不透明度的灰度纹理（黑色：完全透明；白色：完全不透明）。
     * 只使用纹理的颜色，如果存在alpha通道则忽略。对于RGB和RGBA纹理，
     * 由于DXT压缩和未压缩RGB 565格式为绿色提供的额外精度位，
     * 渲染器在采样此纹理时将使用绿色通道。仅亮度和亮度/alpha纹理也将按预期工作。
     ...
     * 当设置为 `MixOperation` 时，使用 {@link MeshBasicMaterial#reflectivity} 在两种颜色之间混合。
    ...
    /**
     * 环境贴图对表面的影响程度。有效范围在 `0`（无反射）和 `1`（完全反射）之间。
  ...
  /**
   * 复制另一个MeshPhongMaterial的属性到当前材质。
   * Copy properties from another MeshPhongMaterial to this material.
   *
   * @param {MeshPhongMaterial} source - 要复制属性的源材质
   * @returns {MeshPhongMaterial} 返回当前材质实例，支持链式调用
   */
  copy(source) {
    // 调用父类的copy方法
    super.copy(source);
