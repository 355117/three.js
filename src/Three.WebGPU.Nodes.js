/**
 * Three.js WebGPU 节点渲染器模块
 *
 * 这是 Three.js 的 WebGPU 节点渲染器入口文件，专门针对节点系统优化的 WebGPU 渲染器。
 * 与标准 WebGPU 渲染器相比，这个版本专门为节点材质系统和计算着色器进行了优化。
 *
 * 主要特性：
 * - 节点优化渲染器：专门为节点系统优化的 WebGPU 渲染引擎
 * - 高级节点材质：支持复杂的节点材质图和着色器组合
 * - 计算着色器集成：深度集成计算着色器功能
 * - 存储缓冲优化：针对节点系统优化的存储缓冲管理
 * - TSL 深度集成：与 Three.js 着色器语言的深度集成
 * - 现代渲染管线：基于节点的现代渲染管线
 *
 * 注意：这个版本相比标准 WebGPU 渲染器缺少某些存储纹理功能，
 * 但在节点材质处理和计算着色器方面有更好的性能。
 *
 * @version r168
...
// WebGPU 节点渲染器 - 专门为节点系统优化的 WebGPU 渲染引擎
// 提供对节点材质和计算着色器的深度优化支持
...
// 存储缓冲属性 - GPU 存储缓冲区属性
// 用于计算着色器的数据存储和交换，针对节点系统优化
export { default as StorageBufferAttribute } from "./renderers/common/StorageBufferAttribute.js";

// 实例化存储缓冲属性 - 支持实例化的存储缓冲属性
// 用于大量实例的高效存储和处理，与节点系统深度集成
